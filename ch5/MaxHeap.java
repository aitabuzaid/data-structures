...
class MaxHeap {
	...
	void insert(int key) {
		...
		while ((curr != 0) && (Heap[curr].compareTo(Heap[parent(curr)]) > 0)) {
			MaxHeap.swap(Heap, curr, parent(curr));
			curr = parent(curr);
		}
	}
	...
	Comparable removemax() {
		...
		MaxHeap.swap(Heap, 0, --n); // Swap maximum with last value
	...
	Comparable remove(int pos) {
		...
		if (pos == (n-1)) n--; // Last element, no work to be done
			...
			MaxHeap.swap(Heap, pos, --n); // Swap with last value
			update(pos);
		}
		return Heap[n];
	}
